...
void draw_map(STATE *s, MAPA *map) {
    ...
    for (int i = 0; i < map->x; i++) {
        ...
        mvaddch(0, i, map->matriz[i][0] | A_BOLD);
        mvaddch(map->y - 2, i, map->matriz[i][map->y - 2] | A_BOLD);
    }
    ...
    for (int i = 0; i < map->y - 1; i++) {
        ...
        mvaddch(i, 0, map->matriz[0][i]| A_BOLD);
        mvaddch(i, map->x - 1, map->matriz[map->x - 1][i]| A_BOLD);
