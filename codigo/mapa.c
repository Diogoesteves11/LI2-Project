...
void draw_map(MAPA* map, int *map_visibility, int rooms, int nrows, int ncols) {
    ...
    for (int i = 0; i < map[i].x; i++) {
        map[i].matrix[i][0] = '#'; 
        map[i].matrix[i][map[i].y - 2] = '#';
        mvaddch(0, i, map[i].matrix[i][0]);
        mvaddch(map[i].y - 2, i, map[i].matrix[i][map[i].y - 2]);
        count1++;
    }
    for (int i = 0; i < map[i].y - 1; i++) {
        map[i].matrix[0][i] = '#'; 
        map[i].matrix[map[i].x - 1][i] = '#'; 
        mvaddch(i, 0, map[i].matrix[0][i]);
        mvaddch(i, map[i].x - 1, map[i].matrix[map[i].x - 1][i]);
        count1++;
    }
   
    for (int i = 1; i < map[i].x - 1; i++) {
        map[i].matrix[i][map[i].y - 1] = ' '; 
        mvaddch(map[i].y - 1, i, map[i].matrix[i][map[i].y - 1]);
    }
    int casas_totais = ((map[i].x) * (map[i].y)) - count1;
    ...
    for (int i = 0; i < casas_totais * 0.4; i++) {
        int x1 = 2 + rand() % (map[i].x - 2); 
        int y1 = 2 + rand() % (map[i].y - 2);
        map[i].matrix[x1][y1] = '#';
        mvaddch(y1, x1, map[i].matrix[x1][y1]);
    }

    for (int i = 0; i < 3; i++) { 
        for (int y = 1; y < (map[i].y-1) - 1; y++) {
            for (int x = 1; x < (map[i].x) - 1; x++) {
                int count = 0;
                for (int yy = y - 1; yy <= y + 1; yy++) {
                    for (int xx = x - 1; xx <= x + 1; xx++) {
                        if (map[i].matrix[xx][yy] == '#') {
                            count++;
                        }
                    }
                }
            ...
            for (int x = 1; x < (map[i].x) - 1; x++) {
                ...
                for (int iy = y - 1; iy <= y + 1; iy++) {
                    for (int ix = x - 1; ix <= x + 1; ix++) {
                        if (map[i].matrix[ix][iy] == '#' && (iy != y || ix != x)) {
                            count++;
                        }
                    }
                }
 ...
 for (int i = 1; i < map[i].x - 1; i++) {
    for (int j = 1; j < map[i].y - 1; j++) {
        if (map[i].matrix[i][j] == '#') {
            int count2 = 0;
            if (map[i].matrix[i - 1][j] == '#') count2++; 
            if (map[i].matrix[i + 1][j] == '#') count2++; 
            if (map[i].matrix[i][j - 1] == '#') count2++; 
            if (map[i].matrix[i][j + 1] == '#') count2++; 
...
while  (trap_count < trap_percentage) {
 int x1 = rand() % (map[i].x - 2) + 1;
 int y1 = rand() % (map[i].y - 1);
 if (map[i].matrix [x1] [y1] != '#') {
  map[i].matrix[x1][y1] = 'x';
  mvaddch(y1, x1, map[i].matrix[x1][y1]);
 }
 trap_count ++;
 }
